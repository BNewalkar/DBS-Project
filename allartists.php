...
<form class="navbar-form navbar-left" role="search" action="allartists.php" style="position:absolute; RIGHT:40%;" method="POST">
     <div class="form-group">
      <input type="text", name="keyword1" style="text-align: center", class="form-control" required = "required" placeholder="Search Artists">
     </div>
...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
  ...
  $query= "select aid,aname,adesc from artists where aname like '%".$keyword1."%'";
  }	
}
else
{
  $query= "select aid,aname,adesc from artists";
}
...
$result=mysqli_query($con,$query);
    ...
    print "<tr class='info'>";
    print "<th style='text-align:center' >Artist Name</th>";
    print "<th style='text-align:center'>Descreption</th>";
    ...
    while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) 
    {
        ...
        print "<tr>";
        print "<td><a href='artist.php?artist=".$aname."'target='_top'>".$aname."</td>";
        print "<td>".$adesc."</td>";
        print "<td>";
       
        $query2 = "select count(*) from likes where userid='$username' and aid = '$aid'";
        ...
        while($row2 = mysqli_fetch_array($result2,MYSQLI_ASSOC)) 
          {
            ...
            if($count >= 1)
            {
                print "<form class='form-horizontal' action='like.php' method='POST'>
                <input type='hidden' name='artist_parameter' value='".$aid."'>
                <button  type='submit' class='btn btn-info' name= 'to_do' value= 'unlike'>Unlike!</button></form>";            
			}
            else
            {
                print "<form class='form-horizontal' action='like.php' method='POST'>
                <input type='hidden' name='artist_parameter' value='".$aid."'>
                <button  type='submit' class='btn btn-info' name= 'to_do' value= 'like'>Like!</button></form>";
            }
